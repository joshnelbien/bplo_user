...
const menuItems = [
  ...
  { text: "New Application", path: "/new_records", icon: <AssignmentTurnedInIcon /> },
  { text: "Renew Application", path: "/renew_records", icon: <AutorenewIcon /> },
  ...
  const drawer = (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Box sx={{ p: 2, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        ...
        <img src="/spclogo.png" alt="SPC Logo" style={{ width: '100px', height: '100px', objectFit: 'contain' }} />
      </Box>
      <List sx={{ p: 2, flexGrow: 1 }}>
        ...
        {menuItems.map((item) => (
          <ListItem key={item.text} disablePadding>
            <ListItemButton
              ...
              sx={location.pathname === item.path ? activeListItemStyle : listItemStyle}
            >
              <ListItemIcon sx={{
        ...
        <ListItemButton onClick={() => setOpenDept(!openDept)} sx={listItemStyle}>
          ...
          {openDept ? <ExpandLess sx={{ color: "#1a7322" }} /> : <ExpandMore sx={{ color: "#1a7322" }} />}
        </ListItemButton>
          ...
          <List component="div" disablePadding>
            {departmentItems.map((dept) => (
              <ListItem key={dept.text} disablePadding>
                <ListItemButton
                  sx={{ pl: 4, ... (location.pathname === dept.path ? activeListItemStyle : listItemStyle) }}
                  onClick={() => {
                    navigate(dept.path);
                    if (isMobile) setOpenDrawer(false);
                  }}
      ...
      <Box sx={{ p: 2, pt: 0 }}>
        <Divider sx={{ my: 1, borderColor: 'transparent' }} /> {/* Removed outline from the divider */}
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Box sx={{
        ...
        <Chip
          ...
          sx={{ color: isOnline ? "white" : "black", backgroundColor: isOnline ? "#4caf50" : "#bdbdbd" }}
        />
      </Box>
      ...
      >
        <Box sx={modalStyle}>
          ...
          <Typography id="logout-modal-description" sx={{ mt: 2, mb: 3, textAlign: 'center' }}>
