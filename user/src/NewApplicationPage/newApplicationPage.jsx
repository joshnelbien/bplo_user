...
import HomeIcon from '@mui/icons-material/Home'; // New import for the Home button
...
  borderRadius: "8px",
  ...(variant === "contained" && {
    backgroundColor: "#4caf50", // Default green (will be overridden locally for Back/Next/Home)
    color: "#fff",
...
  // Changed isMobile to check for 'md' breakpoint to ensure more room for the stepper
  const isMobile = useMediaQuery(theme.breakpoints.down("md")); 
  const isExtraSmall = useMediaQuery(theme.breakpoints.down("sm")); // For Paper maxWidth adjustment
  
  const savedFormData =
...
  useEffect(() => {
    // Only scroll the Paper component if we are not on the success screen (successDialogOpen)
    if (paperRef.current) {
...
        <Box
          sx={{
            // MaxWidth adjusted to use 100% on small screens for better flexibility
            width: "100%", 
            maxWidth: isExtraSmall ? "90%" : 900, // Use 90% width on xs/sm screens
            mx: "auto",
            px: isExtraSmall ? 1 : 0, // Add a little horizontal padding on small screens
            mb: 2,
            display: 'flex', // ADDED: Display flex
            justifyContent: 'center', // MODIFIED: Center the button horizontally
          }}
        >
          <GreenButton
...
            variant="contained"
            sx={{
              // MODIFIED: New Background Color for Home button
              backgroundColor: "#1d5236", 
              // MODIFIED: New Hover Color for Home button
              "&:hover": {
                backgroundColor: "#072b0b", 
              },
              // ADDED: Make button square/circular for icon
...
            }}
          >
            {/* MODIFIED: Replace text with Home icon */}
...
          <Stepper
            activeStep={step - 1}
            // Change orientation to 'vertical' on mobile
            orientation={isMobile ? "vertical" : "horizontal"} 
            alternativeLabel={!isMobile} // Only use alternativeLabel for horizontal
            connector={isMobile ? null : null} // Keep null for both, but usually vertical has a connector
            sx={{
              mb: 2,
              // Stepper styles for horizontal (desktop)
              ...(!isMobile && {
                flexWrap: "nowrap", 
...
                },
              }),
              // Stepper styles for vertical (mobile)
              ...(isMobile && {
                "& .MuiStep-root": {
                  padding: "8px 0", // Add vertical spacing
                },
                "& .MuiStepLabel-root": {
                  textAlign: "left",
                },
...
              // MODIFIED: Step Icon Colors (Gray for incomplete, Green for active/complete)
              "& .MuiStepIcon-root": {
                fontSize: { xs: "1rem", sm: "1.5rem" },
                color: "gray", // Default/Incomplete color is gray
                "&.Mui-active": { color: "#1d5236" }, // Active step is dark green
                "&.Mui-completed": { color: "#1d5236" }, // Completed step is dark green
                "& .MuiStepIcon-text": {
                  // Make the number/check white when the background is dark green
                  fill: "#ffffff", 
                },
              },
              "& .MuiStepLabel-label": {
                // Ensure label is readable on mobile
...
                <GreenButton
                  type="button"
                  variant="contained" // MODIFIED: Changed to contained
                  onClick={() => setStep(step - 1)}
                  sx={{
                    // MODIFIED: Back button colors (Grey)
...
                  sx={{
                    // MODIFIED: Next button colors (Dark Green)
                    backgroundColor: "#1d5236",
                    "&:hover": {
                      backgroundColor: "#072b0b", // darker shade on hover
                    },
                  }}
                >
                  Next
